...
files_gcc = \
	...
	$(shell for h in \
		    ...
		    {,a,b,e,n,p,s,t,x}mmintrin.h mmintrin-common.h \
		    {syslimits,unwind,varargs}.h; \
		do \
...
ifeq ($(biarch64),yes)
	...
	dh_link -p$(p_gcc_m) \
	  ...
	  /$(lib64)/libgcc_s.so.$(GCC_SONAME) /$(gcc_lib_dir)/$(biarch64subdir)/libgcc_s.so
  ifeq ($(with_lib64gmath),yes)
	rm -f $(d)/$(PF)/$(lib64)/libgcc-math.{la,so}
	dh_link -p$(p_gcc_m) \
	  /$(PF)/$(lib64)/libgcc-math.so.$(GCC_SONAME) /$(gcc_lib_dir)/$(biarch64subdir)/libgcc-math.so
	mv $(d)/$(PF)/$(lib64)/libgcc-math.a $(d)/$(gcc_lib_dir)/$(biarch64subdir)/
  endif
endif
ifeq ($(biarch32),yes)
	DH_COMPAT=2 dh_movefiles -p$(p_gcc_m) \
	...
	dh_link -p$(p_gcc_m) \
	  ...
	  /$(lib32)/libgcc_s.so.$(GCC_SONAME)  /$(gcc_lib_dir)/$(biarch32subdir)/libgcc_s.so
  ifeq ($(with_lib32gmath),yes)
	rm -f $(d)/$(lib32)/libgcc-math.{la,so}
	dh_link -p$(p_gcc_m) \
	  /$(lib32)/libgcc-math.so.$(GCC_SONAME) /$(gcc_lib_dir)/$(biarch32subdir)/libgcc-math.so
	mv $(d)/$(lib32)/libgcc-math.a $(d)/$(gcc_lib_dir)/$(biarch32subdir)/
  endif
endif
ifeq ($(biarchn32),yes)
	DH_COMPAT=2 dh_movefiles -p$(p_gcc_m) \
	...
	dh_link -p$(p_gcc_m) \
	  /$(libn32)/libgcc_s.so.$(GCC_SONAME) /$(gcc_lib_dir)/libgcc_s_n32.so \
	  /$(libn32)/libgcc_s.so.$(GCC_SONAME) /$(gcc_lib_dir)/$(biarchn32subdir)/libgcc_s.so
  ifeq ($(with_libn32gmath),yes)
	rm -f $(d)/$(PF)/$(libn32)/libgcc-math.{la,so}
	dh_link -p$(p_gcc_m) \
	  /$(PF)/$(libn32)/libgcc-math.so.$(GCC_SONAME) /$(gcc_lib_dir)/$(biarchn32subdir)/libgcc-math.so
	mv $(d)/$(PF)/$(libn32)/libgcc-math.a $(d)/$(gcc_lib_dir)/$(biarchn32subdir)/
  endif
endif

ifeq ($(with_libssp),yes)
  ifeq ($(biarch64),yes)
	mv $(d)/$(PF)/$(lib64)/libssp*.a $(d_gcc_m)/$(gcc_lib_dir)/$(biarch64subdir)/
	rm -f $(d)/$(PF)/$(lib64)/$(biarch64subdir)/libssp*.{la,so}
	dh_link -p$(p_gcc_m) \
	  /$(PF)/$(lib64)/libssp.so.$(SSP_SONAME) /$(gcc_lib_dir)/$(biarch64subdir)/libssp.so
  endif
  ifeq ($(biarch32),yes)
	mv $(d)/$(lib32)/libssp*.a $(d_gcc_m)/$(gcc_lib_dir)/$(biarch32subdir)/
	rm -f $(d)/$(lib32)/$(biarch32subdir)/libssp*.{la,so}
	dh_link -p$(p_gcc_m) \
	  /$(lib32)/libssp.so.$(SSP_SONAME) /$(gcc_lib_dir)/$(biarch32subdir)/libssp.so
  endif
  ifeq ($(biarchn32),yes)
	mv $(d)/$(PF)/$(libn32)/libssp*.a $(d_gcc_m)/$(gcc_lib_dir)/$(biarchn32subdir)/
	rm -f $(d)/$(PF)/$(libn32)/$(biarchn32subdir)/libssp*.{la,so}
	dh_link -p$(p_gcc_m) \
	  /$(PF)/$(libn32)/libssp.so.$(SSP_SONAME) /$(gcc_lib_dir)/$(biarchn32subdir)/libssp.so
  endif
endif

ifeq ($(with_gomp),yes)
  ifeq ($(biarch64),yes)
	mv $(d)/$(PF)/$(lib64)/libgomp*.a $(d_gcc_m)/$(gcc_lib_dir)/$(biarch64subdir)/
	rm -f $(d)/$(PF)/$(lib64)/$(biarch64subdir)/libgomp*.{la,so}
	dh_link -p$(p_gcc_m) \
	  /$(PF)/$(lib64)/libgomp.so.$(GOMP_SONAME) /$(gcc_lib_dir)/$(biarch64subdir)/libgomp.so
  endif
  ifeq ($(biarch32),yes)
	mv $(d)/$(lib32)/libgomp*.a $(d_gcc_m)/$(gcc_lib_dir)/$(biarch32subdir)/
	rm -f $(d)/$(lib32)/$(biarch32subdir)/libgomp*.{la,so}
	dh_link -p$(p_gcc_m) \
	  /$(lib32)/libgomp.so.$(GOMP_SONAME) /$(gcc_lib_dir)/$(biarch32subdir)/libgomp.so
  endif
  ifeq ($(biarchn32),yes)
	mv $(d)/$(PF)/$(libn32)/libgomp*.a $(d_gcc_m)/$(gcc_lib_dir)/$(biarchn32subdir)/
	rm -f $(d)/$(PF)/$(libn32)/$(biarchn32subdir)/libgomp*.{la,so}
	dh_link -p$(p_gcc_m) \
	  /$(PF)/$(libn32)/libgomp.so.$(GOMP_SONAME) /$(gcc_lib_dir)/$(biarchn32subdir)/libgomp.so
  endif
endif
